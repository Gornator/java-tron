...
@Slf4j(topic = "DB")
@CommandLine.Command(name = "benchmap",
    description = "Benchmark freezer's delay of reading one block.")
public class BenchMapFreezer implements Callable<Integer> {
  ...
  private Map<Integer, MappedByteBuffer> index2Input = new HashMap<>();
  private Map<Integer, Long> index2FileLength = new HashMap<>();
  private byte[] cIdx;
  ...
  public BenchMapFreezer() throws IOException {
    readIndex();
  }

  public BenchMapFreezer(long minBlockNum, long maxBlockNum, int testTimes) throws IOException {
  ...
  public byte[] readBlockNum(long blockNum) throws IOException {
    ...
    MappedByteBuffer mappedByteBuffer = index2Input.get(fileNum);
    if (mappedByteBuffer == null) {
      ...
      mappedByteBuffer = fc.map(MapMode.READ_ONLY, 0, fc.size());

      index2FileLength.put(fileNum, dataFile.length());
      index2Input.put(fileNum, mappedByteBuffer);
    }
    readIndexCost += (System.currentTimeMillis() - t1);

    long t3 = System.currentTimeMillis();
    if (readToEnd) {
      size = (int) (index2FileLength.get(fileNum) - offset);
    }
    byte[] data = new byte[size];
    mappedByteBuffer.position(offset);
    mappedByteBuffer.get(data, 0, size);
